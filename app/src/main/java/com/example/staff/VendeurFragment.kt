...
class VendeurFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        adapter = VendeurAdapter(emptyList())
        ...
        ApiHelper().fetchvendeur { vendeurs ->
            requireActivity().runOnUiThread {
                ...
                adapter = VendeurAdapter(mList)
                if (recyclerview != null) {
                    recyclerview.adapter = adapter
                }

                adapter.setOnClickListener { clientId ->
                    Toast.makeText(requireContext(), "Vendeur Selectionner id: $clientId", Toast.LENGTH_SHORT).show()
